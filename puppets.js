...
async function createBrowsers(n) {
  return await Promise.all([...new Array(n)].map(async (x) => await puppeteer.launch()))
}
...
async function registerAndLogin(pages) {
  for (let i = 0; i < pages.length; i++) {
    ...
    await page.screenshot({path: `./screenshots/register/screenshot${i}.png`});
...
// open multiple pages to registration.
